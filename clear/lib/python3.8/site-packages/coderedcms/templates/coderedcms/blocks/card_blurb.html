...
{% if self.settings.custom_id %}id="{{self.settings.custom_id}}"{% endif %}>
    {% if self.image %}
    {% lazy_image self.image fill-150x150 as card_img %}
    <img class="lazy rounded-circle w-25 mx-auto" data-src="{{card_img.url}}" src="{{card_img.lazy_url}}" alt="{{card_img.title}}">
    {% endif %}
    <div class="card-body">
